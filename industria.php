	<?php
		...
		$consulta="SELECT p.id, p.nom_prod, p.precio, p.categoria, f.web_path, p.imagenes FROM productos AS p INNER JOIN productos_files as pf ON pf.producto_id=p.id INNER JOIN files AS f ON f.id=pf.file_id where categoria='industria' GROUP BY p.id ";
		...
		if($_GET['pagina']>$paginas || $_GET['pagina']<1){
			if($sesion=='ok'){
				header('location:?sesion=ok&m=industria&pagina=1');
			}else{

			header('location:?m=industria&pagina=1');
			}
		}
		...
		$sql_art="SELECT p.id, p.nom_prod, p.precio, p.categoria, f.web_path, p.imagenes FROM productos AS p INNER JOIN productos_files as pf ON pf.producto_id=p.id INNER JOIN files AS f ON f.id=pf.file_id WHERE categoria='industria' GROUP BY p.id LIMIT $iniciar,$artxpag ";
				...
				<div class="container-fluid">
					...
					<nav>
						<ul class="pagination ">
							...
							?>
							">
						      <a class="page-link" href="?<?php if($sesion=='ok'){echo 'sesion=ok&';}?>m=industria&pagina=1" aria-label="Previous">
							...
							?>
							">
								<a class="page-link" href="?<?php if($sesion=='ok'){echo 'sesion=ok&';}?>m=industria&pagina=<?php echo $_GET['pagina']-1; ?>">
									Anterior
								</a>
							</li>
							...
							if ($paginas >= 1) {
					            ...
					            for ($i = $primera; $i <=$ultima; $i++){
					                if ($_GET['pagina']  == $i){
					                	...
					                	?>
					                	<li class="page-item"><a class="page-link" href="?<?php if($sesion=='ok'){echo 'sesion=ok&';}?>m=industria&pagina=<?php echo $i;?>"><?php echo $i; ?></a></li>
							...
							<li class="page-item
							 ...
							 ?>
							 ">
      							<a class="page-link" href="?<?php if($sesion=='ok'){echo 'sesion=ok&';}?>m=industria&pagina=<?php echo $paginas; ?>" aria-label="Next">
